...
kubectl apply -f "$PROJECT_DIR/k8s/G_19_namespace.yaml"
kubectl apply -f "$PROJECT_DIR/k8s/G_19_rbac.yaml"
kubectl apply -f "$PROJECT_DIR/k8s/G_19_apps.yaml"
kubectl apply -f "$PROJECT_DIR/k8s/G_19_linkerd-viz-access.yaml"
...
echo "  1. Start data collection:  ./scripts/G_19_run-experiment.sh"
echo "  2. Wait 2-3 minutes for baseline data"
echo "  3. Start the controller:   kubectl apply -f k8s/G_19_controller.yaml"
