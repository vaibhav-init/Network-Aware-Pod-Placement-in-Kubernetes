...
kubectl apply -f "$PROJECT_DIR/k8s/apps.yaml"
kubectl apply -f "$PROJECT_DIR/k8s/linkerd-viz-access.yaml"
...
echo "  1. Start data collection:  ./scripts/run-experiment.sh"
echo "  2. Wait 2-3 minutes for baseline data"
echo "  3. Start the controller:   kubectl apply -f k8s/controller.yaml"
