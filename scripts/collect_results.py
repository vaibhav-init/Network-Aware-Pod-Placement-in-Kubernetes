...
class ExperimentCollector:
    def __init__(self, args: argparse.Namespace):
        ...
        self.placements_csv = self.session_dir / "placements.csv"
        self.edges_csv = self.session_dir / "service_edges.csv"
        self.deployments_csv = self.session_dir / "deployments.csv"
    ...
    def write_metadata(self):
        with (self.session_dir / "metadata.json").open("w") as handle:
    ...
    def finalize(self):
        ...
        (self.session_dir / "pods-final.json").write_text(json.dumps(pods_json, indent=2))
        (self.session_dir / "deployments-final.json").write_text(
            json.dumps(deployments_json, indent=2)
        )
        (self.session_dir / "controller.log").write_text(
            kubectl_logs(self.args.namespace, "deploy/network-rescheduler", "controller")
        )
        (self.session_dir / "loadgen.log").write_text(
            kubectl_logs(self.args.namespace, "deploy/loadgen", "loadgen")
        )
        (self.session_dir / "events.txt").write_text(kubectl_events(self.args.namespace))

        loadgen_samples = self.parse_loadgen_log(self.session_dir / "loadgen.log")
        move_events = self.parse_controller_log(self.session_dir / "controller.log")
        ...
        with (self.session_dir / "summary.json").open("w") as handle:
    ...
    def write_loadgen_csv(self, samples: List[dict]):
        with (self.session_dir / "loadgen_metrics.csv").open("w", newline="") as handle:
            writer = csv.writer(handle)
    ...
    def write_move_csv(self, events: List[dict]):
        with (self.session_dir / "move_events.csv").open("w", newline="") as handle:
            writer = csv.writer(handle)
    ...
    def write_edge_comparison_svg(self, items: List[dict]):
        ...
        (self.plots_dir / "edge_p95_comparison.svg").write_text("".join(parts))
    ...
    def write_loadgen_latency_svg(self, samples: List[dict], move_events: List[dict]):
        ...
        parts.append("</svg>")
        (self.plots_dir / "loadgen_latency.svg").write_text("".join(parts))
